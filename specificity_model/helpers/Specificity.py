...
class SpecificityFeatureExtraction:
    ...
    def specificity_features(self,dict_sent_counts,corpus_length):
        ...
        for index,text in enumerate(tqdm(self.texts,desc='Collecting Specificity features.',bar_format='{l_bar}{bar} [{n_fmt}/{total_fmt}]')):
            ...
            max_idf = 0
            if len(idfs) > 0:
                avg_idfs = sum(idfs)/len(idfs)
                min_idf = min(idfs)
                max_idf = max(idfs)
            dict_specificity_features['avg_idfs'].append(avg_idfs)
            dict_specificity_features['min_idf'].append(min_idf)
            dict_specificity_features['max_idf'].append(max_idf)    
    ...
    def word_features(self,state='t',sent_type='i'):  #state can be 't' for training or 'p' for prediction  
        ...
        if sent_type == 'i':
            ...
            file_path = '/Users/deepak/Desktop/thesis_pqai/specificity_model/models/count_vec_combined.pickle'
